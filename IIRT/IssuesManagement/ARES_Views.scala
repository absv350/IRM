...
object ARES_Views {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().appName("ARES_Views").master("local").getOrCreate()
    ...
    val mf = spark.read.format("jdbc").option("quote", "\"").option("multiLine", "true").option("wholeFile", "True").option("parserLib", "UNIVOCITY").
      option("driver", "com.microsoft.sqlserver.jdbc.SQLServerDriver").option("url", connectionurl).
